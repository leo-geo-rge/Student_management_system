...
function ViewUser() {
  ...
  async function fetchStudents() {
    try {
      const response = await fetch(
        "https://student-management-system3-pskr.onrender.com/students"
      );
  ...
  async function handleDelete(id) {
    ...
    // ⚠️ avoid naming it 'confirm' — that shadows window.confirm
    if (!confirmed) return;

    try {
      const response = await fetch(
        `https://student-management-system3-pskr.onrender.com/students/${id}`,
        { method: "DELETE" }
      );
      const data = await response.json();
      alert(data.message);
      setStudents((prev) => prev.filter((student) => student._id !== id));
      // ✅ uses functional update — safer than closing over stale 'students'
    } catch (error) {
    ...
    <div className="viewuser-page">
      <div className="viewuser-container">
        ...
        {loading ? (
          ...
          // ✅ table-wrapper added — this is what makes horizontal scroll work
